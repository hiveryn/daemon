...
- Ticket `references` remain a portable string list and may contain same-board ticket IDs or normalized absolute filesystem paths. API ticket views expose daemon-resolved reference type, existence, and file/directory kind. External path references are read-only launch context and never expand writable repo scope; runtimes that cannot enforce this distinction must fail clearly.
