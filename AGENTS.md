...
- `sessionruntime/` also owns the in-memory conclusion approval store (one pending approval per session), resolved per-intent terminal UUIDs, and right-pane tab layout state for the current run — a single tab list shared by terminal tabs (PTY-backed) and browser tabs (`type: "browser"`, no PTY, mutated via `PreviewBrowserTab`/`CloseBrowserTab`), each keyed by its own UUID in `SessionTab.ID`; SQLite stores durable `session_intents`, `session_runs`, and structured events, not terminal identity/layout snapshots or approval state.
