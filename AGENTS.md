...
internal/
  ...
  roadmapfs/          architect roadmap storage (roadmap/current.yaml + archive.yaml): pair-hash version token, atomic op batches, full-state validation, ticket-link resolution
...
- The roadmap pair (`roadmap/current.yaml` + `roadmap/archive.yaml`) is one logical version: a single content hash covers both files, every update validates the complete final state before writing, and both files rename atomically in data direction — archive.yaml first when archiving, current.yaml first when restoring, so an interruption can duplicate but never lose a subtree (which is why one batch may not mix archive and restore). MCP exposes no delete — archive/restore only.
- Agent tool calls that need the user's approval route through the **intent system** (`sessionruntime/intents*.go`): the daemon raises a pending intent, publishes a durable `type: "intent"` / `status: "required"` session event carrying `{intent_id, intent_type, summary, payload, origin, wait_seconds, policy}`, and BLOCKS the agent's MCP call on a channel until the user answers or the tool's policy fires. No polling. The side effect lives in the intent's `Exec` and runs only after the intent resolves approved, so enforcement is daemon-side and an agent cannot bypass it. `concludeSession`, `createWorkTicket`, and architect-only `spawnTicketSession` are routed today; spawn requires an explicit configured profile, revalidates profile and backlog ticket state inside `Exec`, and uses `wait-then-allow` so an unanswered timeout proceeds with the launch. Other mutating tools (ticket edits, `updateArchitectConfig`, the roadmap tools) write directly — they are architect-owned bookkeeping guarded by validation and version tokens, with prompt-level guidance to discuss material changes; routing one later is a one-line `intentPolicies` addition. The desktop's own direct routes (`POST /api/sessions/{id}/conclude`, `POST /api/architects/{key}/tickets`) stay unapproved because approval gates the agent, not the user.
