...
5. Hiveryn does not track edits to selected workflows (no content digests). An active agent is not updated by a file edit; tell it to reread the file. On restore and resume the kickoff is not replayed, so the daemon appends a "Session resumed" section to the instructions carrying the selected workflows' current bodies, read by that resume's validation, stating they replace any earlier copy in the conversation. It never claims the contents are unchanged or falls back to a path-only reminder.
