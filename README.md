...
archives/roadmaps/           # ROADMAP-YYYY-MM-DD[-NN].md, archivedAt frontmatter
```
...
**Ticket (worker) sessions** receive the built-in `WORKER_SYSTEM` instructions (additive; they describe only `createWorkTicket` and `concludeTicketSession`) and a fixed kickoff naming the ticket id, the writable repositories (`key: path`), the canonical paths of the project documents (`PROJECT_OVERVIEW.md`, `PROJECT_STATE.md`, and `ROADMAP_CURRENT.md` only when it exists), and the canonical paths of the workflows selected for the session, or `none`. The worker reads those files live in the architect workspace; the workspace is never added to its writable scope, and no workflow body is copied or embedded.
...
Before offering a launch at all, a client can ask `GET /api/architects/{key}/workspace/worker-preflight` whether the workspace's required project context is ready: it runs the launch's own project-context validation without a selection and answers `launchable` plus one actionable `problems` line per finding. It is deliberately not `checkWorkspace.valid` — architect-only artifacts, archived roadmaps and unselected invalid workflows make that verdict false without blocking a worker.
...
5. Hiveryn does not track edits to selected workflows (no content digests). An active agent is not updated by a file edit; tell it to reread the file. On resume the daemon appends a "Session resumed" note to the instructions telling the agent to reread the selected canonical paths — it never claims their contents are unchanged.
...
| `GET` | `/api/architects/{key}/workspace/artifacts/{kind}` | Describe one artifact kind (`HIVERYN_YAML`, `PROJECT_OVERVIEW`, `PROJECT_STATE`, `ROADMAP_CURRENT`, `ROADMAP_ARCHIVE`, `ARCHITECT_SYSTEM`, `WORKFLOW`): schema version, location/naming, fields, rules, and a minimal example, rendered from the same definitions the check executes. `VALIDATION` for anything else — tickets and conclusions keep their own tools and schemas. Called by the MCP `describeArtifact` tool. |
