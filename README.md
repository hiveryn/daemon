...
| `POST` | `/api/sessions/{id}/intents/execute-action` | **Blocks until the approval resolves (wait-then-allow, like `create-work-ticket`).** Request one execution of an available Action (`{name, prompt, variant}`): records it `pending_approval` with the requested variant and raises the approval. Returns `ExecuteActionResponse` `{outcome, reason, result}`, where `result` is the `ActionResult` under the execution id (also the intent id): `running` once approved or auto-approved and launched, `denied`, or `failed` when it could not start. `VALIDATION` if the variant is missing or not configured (the message lists every configured variant and tells the agent to ask the user) or the Action is not available to this project or invalid; `CONFLICT` if it is running. Called by the MCP `executeAction` tool. |
...
Agent tool calls that mutate user-visible state route through the **intent system** so the desktop user can approve them before they take effect. Three blocking tools are routed today: the conclude tools (`concludeArchitectSession`, `concludeTicketSession`, role-scoped so a session only sees its own), `createWorkTicket` (registered for both session types) and `executeAction` (architects and ticket workers) — see "Agent-requested Actions". Each takes **discrete structured fields** rather than a free-text body; the daemon renders/validates them and, for conclusions, produces the canonical `conclusion.md`. See "Structured conclusions" below.
...
- Results are read from the durable execution record (never pruned), scoped to the project: a later session of the same project can read them, another project cannot. The variant is part of the request identity: an identical request (same name, prompt and variant) from the same session attaches to the original while it is pending and replays its outcome for 10 minutes after it resolves; a request for another variant is a new request. After the window a new request is created (the single-run rule still applies).
