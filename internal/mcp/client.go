...
func (s *Server) previewInBrowserTab(ctx context.Context, target, tabID string) (PreviewInBrowserTabOutput, error) {
	...
	bodyBytes, err := json.Marshal(body)
	if err != nil {
		return PreviewInBrowserTabOutput{}, newInternalError(fmt.Sprintf("marshal previewInBrowserTab body: %v", err))
	}

	u := fmt.Sprintf("%s/api/sessions/%s/browser-tabs", s.daemonURL, url.PathEscape(s.sessionID))
	req, err := http.NewRequestWithContext(ctx, http.MethodPost, u, bytes.NewReader(bodyBytes))
	if err != nil {
		return PreviewInBrowserTabOutput{}, fmt.Errorf("build previewInBrowserTab request: %w", err)
	}
	req.Header.Set("Content-Type", "application/json")

	resp, err := s.httpClient.Do(req)
	if err != nil {
		return PreviewInBrowserTabOutput{}, fmt.Errorf("request previewInBrowserTab: %w", err)
	}
	defer func() {
		_ = resp.Body.Close()
	}()
	...
	var env daemonEnvelope
	if err := json.NewDecoder(resp.Body).Decode(&env); err != nil {
		return PreviewInBrowserTabOutput{}, newInternalError(fmt.Sprintf("decode daemon response: %v", err))
	}

	if env.Error != nil {
		return PreviewInBrowserTabOutput{}, mapDaemonError(env.Error)
	}
	if len(env.Data) == 0 {
		return PreviewInBrowserTabOutput{}, newInternalError("daemon response missing data")
	}
	if err := json.Unmarshal(env.Data, &output); err != nil {
		return PreviewInBrowserTabOutput{}, newInternalError(fmt.Sprintf("decode browser tab payload: %v", err))
	}

	return PreviewInBrowserTabOutput{TabID: output.TabID, Target: output.Target}, nil
}

func (s *Server) getBrowserTabs(ctx context.Context) (GetBrowserTabsOutput, error) {
	var tabs []domain.SessionTab

	u := fmt.Sprintf("%s/api/sessions/%s/tabs", s.daemonURL, url.PathEscape(s.sessionID))
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, u, nil)
	if err != nil {
		return GetBrowserTabsOutput{}, fmt.Errorf("build getBrowserTabs request: %w", err)
	}

	resp, err := s.httpClient.Do(req)
	if err != nil {
		return GetBrowserTabsOutput{}, fmt.Errorf("request getBrowserTabs: %w", err)
	}
	defer func() {
		_ = resp.Body.Close()
	}()
	...
	var env daemonEnvelope
	if err := json.NewDecoder(resp.Body).Decode(&env); err != nil {
		return GetBrowserTabsOutput{}, newInternalError(fmt.Sprintf("decode daemon response: %v", err))
	}

	if env.Error != nil {
		return GetBrowserTabsOutput{}, mapDaemonError(env.Error)
	}
	if err := json.Unmarshal(env.Data, &tabs); err != nil {
		return GetBrowserTabsOutput{}, newInternalError(fmt.Sprintf("decode tabs payload: %v", err))
	}

	output := GetBrowserTabsOutput{Tabs: []BrowserTabSummary{}}
	for _, tab := range tabs {
		if tab.Type != "browser" {
			continue
		}
		output.Tabs = append(output.Tabs, BrowserTabSummary{TabID: tab.ID, Target: tab.Target})
	}
