...
type ReadRoadmapInput struct {
	View  string `json:"view,omitempty" jsonschema:"current (default) or archive"`
	ID    string `json:"id,omitempty" jsonschema:"Focus: a current item ID, or an archive entry root_id"`
	Depth *int   `json:"depth,omitempty" jsonschema:"Current view only: levels below the focus (0 = focused item(s) only); omit for full subtree"`
}

type ReadRoadmapOutput = domain.RoadmapView

// UpdateRoadmapOutput is the shared result of every roadmap mutation tool:
// the next version token, what changed, resolved tickets, warnings.
type UpdateRoadmapOutput = domain.RoadmapUpdateResult

type CreateRoadmapItemInput struct {
	Version         string   `json:"version" jsonschema:"Token from readRoadmap or the last mutation response; stale = conflict, re-read"`
	ID              string   `json:"id" jsonschema:"kebab-case, max 64 chars, unique forever (archived IDs stay reserved)"`
	Kind            string   `json:"kind" jsonschema:"goal, initiative, or milestone; nesting order goal > initiative > milestone (same kind may nest)"`
	Title           string   `json:"title" jsonschema:"Max 200 chars"`
	Outcome         string   `json:"outcome" jsonschema:"Intended result, not a checklist; max 2000 chars"`
	Status          string   `json:"status,omitempty" jsonschema:"planned (default), active, blocked, or done"`
	ParentID        string   `json:"parent_id,omitempty" jsonschema:"Parent item ID; omit for top level"`
	Order           *int     `json:"order,omitempty" jsonschema:"Position among siblings; omit to append"`
	SuccessCriteria []string `json:"success_criteria,omitempty" jsonschema:"Non-empty strings, no duplicates"`
	DependsOn       []string `json:"depends_on,omitempty" jsonschema:"Current item IDs; not ancestors/descendants, no cycles"`
}

type UpdateRoadmapItemInput struct {
	Version         string    `json:"version" jsonschema:"Token from readRoadmap or the last mutation response; stale = conflict, re-read"`
	ID              string    `json:"id" jsonschema:"Item to change"`
	Title           *string   `json:"title,omitempty" jsonschema:"Max 200 chars"`
	Status          *string   `json:"status,omitempty" jsonschema:"planned, active, blocked, or done"`
	Outcome         *string   `json:"outcome,omitempty" jsonschema:"Max 2000 chars"`
	SuccessCriteria *[]string `json:"success_criteria,omitempty" jsonschema:"Full replacement list"`
	DependsOn       *[]string `json:"depends_on,omitempty" jsonschema:"Full replacement list of current item IDs"`
}

type MoveRoadmapItemInput struct {
	Version  string  `json:"version" jsonschema:"Token from readRoadmap or the last mutation response; stale = conflict, re-read"`
	ID       string  `json:"id" jsonschema:"Item to move"`
	ParentID *string `json:"parent_id,omitempty" jsonschema:"New parent item ID; empty string = top level; omit to keep the current parent (pure reorder). Kind nesting order applies"`
	Order    *int    `json:"order,omitempty" jsonschema:"Position among siblings; omit to append"`
}

type LinkRoadmapTicketInput struct {
	Version  string `json:"version" jsonschema:"Token from readRoadmap or the last mutation response; stale = conflict, re-read"`
	ID       string `json:"id" jsonschema:"Roadmap item"`
	TicketID string `json:"ticket_id" jsonschema:"Board ticket ID (copy from listTickets)"`
}

type UnlinkRoadmapTicketInput = LinkRoadmapTicketInput

type ArchiveRoadmapItemInput struct {
	Version string `json:"version" jsonschema:"Token from readRoadmap or the last mutation response; stale = conflict, re-read"`
	ID      string `json:"id" jsonschema:"Item to archive (its whole subtree goes with it)"`
	Summary string `json:"summary,omitempty" jsonschema:"Concise closure context; max 500 chars"`
}

type RestoreRoadmapItemInput struct {
	Version  string  `json:"version" jsonschema:"Token from readRoadmap or the last mutation response; stale = conflict, re-read"`
	ID       string  `json:"id" jsonschema:"Archive entry root_id"`
	ParentID *string `json:"parent_id,omitempty" jsonschema:"Replacement parent when the original is gone; empty string = top level; omit to use the original"`
	Order    *int    `json:"order,omitempty" jsonschema:"Position among siblings; omit to use the original"`
}

type SetRoadmapTitleInput struct {
	Version string `json:"version" jsonschema:"Token from readRoadmap or the last mutation response; stale = conflict, re-read"`
	Title   string `json:"title" jsonschema:"Roadmap-level title; max 200 chars"`
}
